...
public class MergeGpxJsonParser {

    private static final TypeReference<List<GpxPoint>> GPXPOINT_LIST_TYPE =
        new TypeReference<>() {};

    private final ObjectMapper objectMapper;

    public List<GpxPoint> deserialize(String jsonContent) {
        if (jsonContent == null || jsonContent.isBlank()) {
            return Collections.emptyList();
        }

        try {
            return objectMapper.readValue(jsonContent, GPXPOINT_LIST_TYPE);
